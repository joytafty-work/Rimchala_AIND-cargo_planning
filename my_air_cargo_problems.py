...
class AirCargoProblem(Problem):
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        kb = PropKB()
        # encode current state pos sentence into the knowledge base
        kb.tell(decode_state(node.state, self.state_map).pos_sentence())

        # check all the goal states, incrementing count if goal is not in knowledge base
        for clause in self.goal:
            if clause not in kb.clauses:
                count += 1
        return count
...
def air_cargo_p2() -> AirCargoProblem:
    ...
    neg = [expr('At(C3, SFO)'), expr('At(C3, JFK)'),
...
def air_cargo_p3() -> AirCargoProblem:
    ...
    neg = [expr('At(C4, SFO)'), expr('At(C4, JFK)'), expr('At(C4, ATL)'),
           ...
           expr('At(P1, JFK)'), expr('At(P1, ATL)'), expr('At(P2, ORD)'),
           ]
    init = FluentState(pos, neg)
    goal = [expr('At(C1, JFK)'),
            expr('At(C2, SFO)'),
