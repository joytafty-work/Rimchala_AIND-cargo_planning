...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # Check if preconditions of the proposed action_node are in current literal levels
            if action_node.prenodes.issubset(self.s_levels[level]):
                # add action node
                self.a_levels[level].add(action_node)

                # Connect action_node to literal node(s) in the previous layer
                for pre in action_node.prenodes:
                    for s_node in self.s_levels[level]:
                        if pre.__eq__(s_node):
    ...
    def add_literal_level(self, level):
        ...
        # Iterate current a_level's effnodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for e2 in node_a2.action.effect_add:
            if e2 in node_a1.action.effect_rem:
                return True
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        opposing_sign = (node_s1.is_pos != node_s2.is_pos)
        return (same_symbol) and (opposing_sign)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        mutex_status = True
        # mutex_status remains True iff all p1, p2 combinations are mutex's
        for p1 in node_s1.parents:
            for p2 in node_s2.parents:
                mutex_status = (mutex_status) and p1.is_mutex(p2)
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            ...
            while not found and level < len(self.s_levels):
                node_idx = 0
                nodes = list(self.s_levels[level])
                while not found and node_idx < len(nodes):
                    s_node = nodes[node_idx]
                    found = (g == s_node.symbol) and s_node.is_pos
                    node_idx += 1
                level += 1
            # Found in this level before incrementing
            level_sum += level-1
